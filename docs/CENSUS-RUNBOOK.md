...
The side panel resets to the top and binds the run to that route and content-script document instance. Every reset and post-scroll probe must present the same in-memory binding together with repeated stable scroll metrics and a safe rendered-card fingerprint. Each round is two-phase: the content script first observes and proposes an action without scrolling; the panel completes the immutable round download; only then may a separate one-shot apply message move the same run, round, bound page surface, scroller, and unchanged observed viewport. Navigation, reload, or binding drift pauses instead of adopting the new surface. The next observation again waits for bounded rendered-state stability at the expected scroll position. A surface that does not settle within the bounded wait pauses rather than being called observed. The route/document binding is control state and is never written into a round segment. Each completed file contains that round's raw card observations and the exact checkpoint needed for restart:
